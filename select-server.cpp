...
extern void getETData(char * strToSend, int * returnSize);
...
void AcceptConnections(SOCKET ListeningSocket)
{
    ...
    while (1) {
        ...
        if (select(0, &ReadFDs, &WriteFDs, &ExceptFDs, &tmval) > 0) {
            ...
            if (FD_ISSET(ListeningSocket, &ReadFDs)) {
				...
				FD_CLR(ListeningSocket, &ExceptFDs);
				FD_CLR(ListeningSocket, &ReadFDs);
                if (sd != INVALID_SOCKET) {
                    ...
                    cout << "Accepted connection from " <<
                            ...
                            ", socket " << sd << "." << endl;
					//FD_SET(sd, &ReadFDs); //Added by Georgi
					FD_SET(sd, &WriteFDs);//Added by Georgi
            ...
            while (it != gConnections.end()) {
                ...
                if (FD_ISSET(it->sd, &ExceptFDs)) {
                    ...
                    if (FD_ISSET(it->sd, &WriteFDs)) {
						...
						getETData((*it).acBuffer, &((*it).nCharsInBuffer));
						if (&((*it).nCharsInBuffer) != 0) {
							bOK = WriteData(*it);
						}
                ...
                if (!bOK) {
					...
					it = gConnections.begin();
					return;
					//FD_SET(ListeningSocket, &ReadFDs); //Added by Georgi
					//FD_SET(ListeningSocket, &ExceptFDs);//Added by Georgi
                }
                else {
